...
static const char  *READ_FORMATS[] = {"Jabali","Nagual","XYZ",NULL};

static const char *WRITE_FORMATS[] = {"XYZ", NULL};

Coral::Coral( QApplication *iapp, int argc, char** argv )
     : QMainWindow( 0 )
{
  app = iapp;
...
void Coral::Report( const char *fmt, ... )
{
  va_list args;   
  char    s[256];
...
void Coral::About()
{
  ...
  QMessageBox::about( this, "About Coral",str);
}

void Coral::ChangeModel( const QString &newmodel )
{
...
void Coral::MakeStyle( const QString &style )
{
...
void Coral::DoReadFile(char* filename, char* format)
{
...
void Coral::WriteImage()
{ viewer->image_render->show(); }

void Coral::NewMolecule()
{
  NATOM = 0;
  viewer->Redraw();
  geoeditor->Update();
...
void Coral::ChangeAtomColor()
{
  coral->es->exec(); 
  int sel = 54; //coral->es->GetSelectedElement();
  QColor color; 
...
void Coral::ChangeSurfaceColor()
{
  coral->SetCursor( "Selection" );
  coral->status |= SFSURF;
  Report("Select a surface");
}

void Coral::LaunchSCFWin( void ) 
{ 
  if (coral)
  {
    if (coral->scfwin) 
...
void Coral::LaunchFREQWin( void ) 
{ 
  if (coral)
  {
    if (coral->freqwin) 
    {
      coral->freqwin->Launch(); 
    }
  }
}

void Coral::SetupMeasurement( const QString &s )
{
...
void Coral::LaunchGeoEditor() { geoeditor->Launch(); }


void Coral::SetCursor( const char *type )
{
